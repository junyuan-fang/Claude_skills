...
trigger_keywords: ["NVIDIA 每日新闻", "英伟达推送", "英伟达新闻推送", "nvidia daily", "nvidia daily news", "nvidia 新闻 cron", "深度版新闻推送"]
...
2. 每条做尽调式扩展（30 秒掌握 why-it-matters）：
   - 公司/产品背景（1 句）
   - 技术细节（参数量/训练数据/benchmark/硬件平台 GB300/Rubin/Blackwell 等）
   - 数据/财务指标（金额/产能/目标价/股价反应）
   - 业内意义/对比（vs AMD/Broadcom/华为昇腾/SK 海力士等）
3. 完整深度版（全细节+所有引用）写到 `/home/xinmiao/code/claude_bot/news_archive/nvidia-$(date +%Y-%m-%d).md`，归档不限字数

### Step 2: 准备两段推送文字
4. **第 1 段 头条快报（≤700 字符）**：开头 1 句「今日 NVIDIA 主线」+ 5 条要点。每条 = 标题 + 1 句关键数据 + 1 句「是什么/为什么」+ 1 句公司背景。末尾标注「(深度细节见第 2 段)」
5. **第 2 段 深度展开（≤700 字符）**：挑当日最具信息量的 1-2 个 item 做 deep dive，必带「公司是什么 / 技术怎么做（训练/数据/benchmark/推理细节其一）/ 数字 / 1 句业内对比」+ 末尾 2-3 个权威源链接
6. 两段口语化，少并列符，多用具体数字（x 倍、y 万、z 亿），少堆词
...
13. `sleep 30`
14. `cc-send-safe --image /tmp/nvidia_daily.jpg`（一次性，失败一次就放弃，绝不重试）

### Step 5: 静默收尾
15. 不发任何 reply 状态消息（无「已推送/任务完成/归档路径」）
16. 本地确认：`ls -la /home/xinmiao/code/claude_bot/news_archive/ | tail -3`（仅 stdout，不发飞书/微信）
