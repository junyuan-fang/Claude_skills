...
- 论文流水线产物汇总要点明"池子大小是否异常"（如 arXiv 429/超时只有 4-8 篇），让用户能识别数据源异常
- daily-papers 汇总要列必读/值得看/可跳过分桶数（如 "20 篇，必读 3 / 值得看 13 / 可跳过 4"）+ MOC 刷新具体数（如 "concept MOC 288 篇、paper MOC 20 篇"）；零必读日也要老实说明并指出原因（如 HF Trending 多为 LLM/T2I 跨界）
...
- 每日 NVIDIA 新闻图文推送，cron ID `a285150d`，工作日 `0 8 * * 1-5`（v3.1 静默深度版：头条 + 深度 + 单图，三段间隔 30s）；6/04-6/19 多日临时调到 07:00 跑过，响应区间 48-387s（6/16 用时 387s 为近期峰值，6/19 用时 226s）
- 每日具身智能新闻图文推送，cron ID `d667c0db`，工作日 `0 12 * * 1-5`（v1.3：行业 + 论文混编，论文必须 14 天内，HF daily 优先，arXiv ID YYMM 粗筛 + abstract 精校），响应 92-261s；6/12、6/15、6/16、6/18 未跑 embodied（仅 NVIDIA + daily-papers），6/17 临时 07:05 跑过用时 239s，6/19 临时 07:05 跑过用时 261s
...
- 已跑通日：6/04（21 篇/3 笔记 minWM+OmniDreams+GN0，1392s）、6/05（8 篇受 arXiv 429/2 笔记 Cosmos3+GRAIL，1268s）、6/08（20 篇/必读3/笔记 PiL-World+WLA+DexFuture，concept 152/paper 8，1447s）、6/09（20 篇/必读3/笔记 STRIPS-WM+AdaWAM+LARA，concept 190/paper 11，1269s）、6/10（19 篇/必读3/笔记 LatentSpatialMemory+Video2Sim2Real+MotionWAM，concept 235/paper 14，1164s）、6/11（19 篇/必读3/笔记 WorldOlympiad+TacForeSight+ManiSplat，concept 288/paper 17，1174s）、6/12（20 篇/必读3/笔记 WMSD+EmbodiedR1.5+APT，concept 288/paper 20，970s）、6/15（异常日：arXiv API 超时，HF Trending 仅 8 篇 LLM/T2I 跨界，推荐文件仅 4 篇/必读 0/笔记 0，concept 288/paper 20 不变，672s）、6/16（19 篇/必读3/笔记 ContactWorld+Mu0+WEAVER，concept 288/paper 23，1159s）、6/17（19 篇/必读3/笔记 DreamXWorld+GAM+QwenRobotWorld，concept 288/paper 26，2345s，最长耗时记录）、6/18（20 篇/必读3/笔记 ERQAPlus+WAMRL+LoopWM，concept 288/paper 29，1127s）、6/19（20 篇/必读3/笔记 MemWorld+OCRR+DREAMChunk，concept 288/paper 32，899s）
- 关注方向包含 World Action Model (WAM)、Physical AI、VLA、Diffusion Policy、Sim2Real、τ0-WM 世界模型、Critic 驱动 RL 后训练、tactile foresight、3D Gaussian Splatting 操作、contact-rich manipulation 世界模型、loop world model、WAM+RL 后训练融合、记忆增强世界模型、对象中心世界模型
- 持续探索微信图片接口节流规律，做频率 vs 日累计的对照实验
- 已开始使用飞书通道（session id `ou_…` 前缀），可在飞书端触发任务；飞书直传 mp4 会报 `code=230055`，需走封面图 + zip 打包
- 两个日推 cron 长期稳定运行，响应时间 48–387 秒区间，无中间态噪音
...
- daily-papers MOC 规模参考（增长趋势）：concept MOC 152 篇（6/08）→ 190（6/09）→ 235（6/10）→ 288（6/11，6/12、6/15、6/16、6/17、6/18、6/19 均持平）/ paper MOC 8（6/08）→ 11（6/09）→ 14（6/10）→ 17（6/11）→ 20（6/12，6/15 持平）→ 23（6/16）→ 26（6/17）→ 29（6/18）→ 32（6/19）；concept MOC 在 288 出现持续饱和平台期（已稳定 9 天），paper MOC 持续线性增长（每个有效日 +3）
- daily-papers 流水线耗时区间 672-2345s（约 11-39 分钟），6/15 仅 672s 为目前最快（因池小+无笔记），6/17 用时 2345s 为目前最长（笔记生成阶段更耗时），6/19 用时 899s 为近期较快
...
- daily-papers 跑完只回一句话汇总：推荐文件路径 + 重点笔记篇数（含必读分桶数）+ 目录页是否刷新（含因 Agent 自带行为导致的"被动刷新"也要明示，附 concept/paper MOC 篇数）；若 fetch 池异常偏小（如 arXiv 429/超时）要点明原因，零必读零笔记日（如 6/15）也要老实说明并指出原因（HF Trending 多为 LLM/T2I 跨界 / 无新 robotics 论文）
