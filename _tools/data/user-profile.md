...
- 每日 NVIDIA 新闻图文推送，cron ID `a285150d`，工作日 `0 8 * * 1-5`（v3.1 静默深度版：头条 + 深度 + 单图，三段间隔 30s）；6/04-6/18 多日临时调到 07:00 跑过，响应区间 48-387s（6/16 用时 387s 为近期峰值）
- 每日具身智能新闻图文推送，cron ID `d667c0db`，工作日 `0 12 * * 1-5`（v1.3：行业 + 论文混编，论文必须 14 天内，HF daily 优先，arXiv ID YYMM 粗筛 + abstract 精校），响应 92-249s；6/12、6/15、6/16、6/18 未跑 embodied（仅 NVIDIA + daily-papers），6/17 临时 07:05 跑过用时 239s
...
- 已跑通日：6/04（21 篇/3 笔记 minWM+OmniDreams+GN0，1392s）、6/05（8 篇受 arXiv 429/2 笔记 Cosmos3+GRAIL，1268s）、6/08（20 篇/必读3/笔记 PiL-World+WLA+DexFuture，concept 152/paper 8，1447s）、6/09（20 篇/必读3/笔记 STRIPS-WM+AdaWAM+LARA，concept 190/paper 11，1269s）、6/10（19 篇/必读3/笔记 LatentSpatialMemory+Video2Sim2Real+MotionWAM，concept 235/paper 14，1164s）、6/11（19 篇/必读3/笔记 WorldOlympiad+TacForeSight+ManiSplat，concept 288/paper 17，1174s）、6/12（20 篇/必读3/笔记 WMSD+EmbodiedR1.5+APT，concept 288/paper 20，970s）、6/15（异常日：arXiv API 超时，HF Trending 仅 8 篇 LLM/T2I 跨界，推荐文件仅 4 篇/必读 0/笔记 0，concept 288/paper 20 不变，672s）、6/16（19 篇/必读3/笔记 ContactWorld+Mu0+WEAVER，concept 288/paper 23，1159s）、6/17（19 篇/必读3/笔记 DreamXWorld+GAM+QwenRobotWorld，concept 288/paper 26，2345s，最长耗时记录）、6/18（20 篇/必读3/笔记 ERQAPlus+WAMRL+LoopWM，concept 288/paper 29，1127s）
- 关注方向包含 World Action Model (WAM)、Physical AI、VLA、Diffusion Policy、Sim2Real、τ0-WM 世界模型、Critic 驱动 RL 后训练、tactile foresight、3D Gaussian Splatting 操作、contact-rich manipulation 世界模型、loop world model、WAM+RL 后训练融合
...
- daily-papers MOC 规模参考（增长趋势）：concept MOC 152 篇（6/08）→ 190（6/09）→ 235（6/10）→ 288（6/11，6/12、6/15、6/16、6/17、6/18 均持平）/ paper MOC 8（6/08）→ 11（6/09）→ 14（6/10）→ 17（6/11）→ 20（6/12，6/15 持平）→ 23（6/16）→ 26（6/17）→ 29（6/18）；concept MOC 在 288 出现持续饱和平台期（已稳定 8 天），paper MOC 持续线性增长
