...
- 论文笔记已支持双 vault 同步
- 3D 点云分割与具身模型跟进:Xiaomi-Robotics-1、DriftWorld、FastWAM、GigaWorldPolicy、AlayaWorld、MVA、ABot-World-0、HOST、Orca、FlowWAM、Kairos、DataPyramid、FeelWorld、HiFi-UMI、DC-WAM、πR²、TurboVLA、CheckVLA、Auto-JEPA、WCM、Faster-WAM、Ego2Robot、WorldCycle、BridgeVLA++、WorldTrace、WorldSimProbe、EchoWM 等
- 论文笔记支持复用:同一论文多日再推时复用已有笔记(Xiaomi-Robotics-1、AlayaWorld、MVA、DataPyramid、ABot-World-0、πR² 均已多次复用),不重复生成
- 必读笔记通常每日 2 篇、350-780 行/篇;单篇可长至 811-966 行(Kairos 811、WorldSimProbe 864、EchoWM 894、WorldTrace 966)
- 笔记生成可交由后台 agent 并行(单日常见 5 个 agent),主流程需逐个回收回报并做行数/完整性校验后才算收尾
- 笔记完整性校验包含固定小节存在性检查(`## 关键公式`、`## 实验结果`)与链接/目录页一致性
...
- 2026-08-14 至 08-25 为"仅两档"稳定期(NVIDIA + 具身),论文推荐档长期未触发;2026-08-26 论文推荐档以一句话短 prompt 形式恢复,且改在 07:15 之后紧随 NVIDIA 档
- 论文推荐档已简化为一句话调用:"走完 daily-papers 三步流水线,静默,完成后一句话报路径 + 笔记篇数 + 目录页刷新状态"
- 周末(8/22、8/23)未见触发记录,说明两档稳定期以工作日为主
- 部分日期只跑 NVIDIA 单档(2026-08-18),说明日程可临时缩减
...
- 用量配额耗尽是常见失败模式:2026-08-07 论文推荐与具身新闻两档均报"You've hit your limit · resets 8:30am",07:00-08:30 密集三连任务易撞额度上限
- 三连任务窗口拥挤:NVIDIA 档单次 267-1896 秒、具身档 219-1279 秒、论文档 1400-2530 秒,叠加后极易在 08:30 额度重置前耗尽配额
- NVIDIA 档超时可挤占后续档:2026-08-18 NVIDIA 跑 1533 秒(至 07:49),当日具身档未触发
- NVIDIA 档耗时波动大(8/18 1533 秒、8/21 1896 秒、8/24 1370 秒、8/25 1190 秒、8/26 846 秒),需预留至 07:45 之后;偶有短耗时(8/17 267 秒)
- 具身档耗时波动大(8/19 219 秒、8/20 705 秒、8/21 756 秒、8/24 1279 秒、8/25 875 秒),典型 700-1300 秒,可跑到 08:00 之后
- 论文推荐档紧接 NVIDIA 档时耗时可达 2530 秒(2026-08-26),两档串跑会占满整个 07:00-08:00 窗口
- 定时任务执行结果需事后核对:多次出现响应错位、空响应(2026-08-12 NVIDIA 档仅回零宽字符)、只回响应时间无内容摘要的情况
- 流水线耗时波动大(1 秒~2760 秒),超长耗时可能与响应异常相关,长时间无回复不代表失败;异常短耗时(如 2026-08-19 具身档 219 秒)同样需核对归档是否真正生成
- 新闻类任务只回"响应时间 X 秒"无摘要已成常态(2026-07-27 起持续至 8/26),需人工核对归档文件与实际推送是否真正执行
- 论文推荐档末尾回报常变成单篇笔记的校验确认(如 EchoWM 894 行、WorldTrace 966 行、WorldSimProbe 864 行),而非用户要求的一句话三要素摘要——需注意按格式回报路径 + 笔记篇数 + 目录页状态
