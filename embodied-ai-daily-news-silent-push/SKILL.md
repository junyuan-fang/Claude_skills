...
description: 每日推送具身智能行业 + 论文混编摘要：三段式静默推送，论文必须 14 天内且带 Project 行，跳过用户已熟概念解释。
trigger_keywords: ["具身智能 新闻", "embodied AI daily", "人形机器人 推送", "VLA 论文推送", "embodied-TODAY.md", "cc-send-safe"]
...
2. **论文日期硬校验（14 天窗口）**——按优先级：
   - a) HuggingFace daily papers (https://huggingface.co/papers) 当周精选，**最优先**（每日 curated）
   ...
   引用更老的论文必须在标题旁标注真实日期（例：「HEX（2026-04-09 旧作，已超 14 天）」），且只能放归档「延伸阅读」，不进头条和 deep dive。**搜不到 14 天内新论文，宁可只放 4-5 条行业、彻底放弃论文项。**

3. 每条做尽调式扩展（30 秒掌握 why-it-matters）：
   - **公司背景（必含）**：公司是谁、做什么、谁创立、什么估值/规模。
   - **概念解释严格 gating**：
     - 用户已熟、**禁止解释**：VLA、WAM、VLM、Sim2Real、Teleoperation、Diffusion Policy、Imitation Learning、RL、Behavior Cloning、Foundation Model、Whole-body Control、ZMP、MPC、CMA-ES、四足、人形 humanoid、dexterous manipulation、locomotion。
     - 只对真正新生/小众术语 1 句带过：论文内部命名（DreamZero、Helix、GR00T-Mimic）、新 method/dataset 名、特殊 benchmark 名（MolmoSpaces、RoboArena）。
...
7. **第 2 段 — 深度展开（≤800 字符）**：1-2 个 item deep dive。必带：是什么 / 怎么做 / 数字 / 业内对比。论文 deep dive：模型架构 1 句、训练数据 1 句、benchmark 1 句、对标 SOTA 1 句、能否落地 1 句——直接用术语。末尾附 2-3 个权威源链接。
8. 口语化，少并列符，多具体数字。

### Step 3: 1 张配图

9. 抓最具代表性的 1 张图（机器人产品图 / CEO 照 / demo 截图 / 论文 architecture 图）→ `/tmp/embodied_daily.jpg`。
